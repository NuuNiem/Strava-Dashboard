...
def calculate_statistics(activities):
    ...
    pb_5km = (
        activities[activities["distance_km"] >= 5]
        .nsmallest(1, "moving_time")["moving_time"]
        .values[0]
    )
    pb_10km = (
        activities[activities["distance_km"] >= 10]
        .nsmallest(1, "moving_time")["moving_time"]
        .values[0]
    )
    pb_half_marathon = (
        activities[activities["distance_km"] >= 21.0975]
        .nsmallest(1, "moving_time")["moving_time"]
        .values[0]
    )
    pb_marathon = (
        activities[activities["distance_km"] >= 42.195]
        .nsmallest(1, "moving_time")["moving_time"]
        .values[0]
    )
    average_pace = (
        (activities["moving_time"].sum() / activities["distance_km"].sum())
        if activities["distance_km"].sum() > 0
        else 0
    )

    month_with_most_activities = (
        activities.resample("M", on="date").size().idxmax().strftime("%B %Y")
    )
...
def create_map_layers(min_distance):
    ...
    for _, row in filtered_activities.iterrows():
        if row["coordinates"]:
            lat_lng = [
                {"lat": coord[0], "lng": coord[1]} for coord in row["coordinates"]
            ]
            layers.append(
                dl.Polyline(
                    positions=lat_lng,
                    ...
                    children=[
                        dl.Tooltip(row["name"]),
                        dl.Popup(
                            "Distance: {:.2f} km, Elevation: {} m".format(
                                row["distance_km"], row["elevation_gain"]
                            )
                        ),
                    ],
                )
        ...
        dbc.Row(
            [
                dbc.Col(
                    [
                        html.H4(
                            "Map of Activities", className="text-center mb-3 header"
                        ),
                        leaflet_map,
                        html.Div(
                            [
                                ...
                                dcc.Slider(
                                    ...
                                    value=0,
                                    marks={
                                        int(i): "{} km".format(int(i))
                                        for i in range(
                                            0,
                                            int(activities["distance_km"].max()) + 1,
                                            10,
                                        )
                                    },
                                ),
                        ...
                        dbc.Card(
                            dbc.CardBody(
                                [
                                    ...
                                    html.P(
                                        "Total Distance: {:.2f} km / 1000 km".format(
                        ...
                        dbc.Card(
                            dbc.CardBody(
                                [
                                    ...
                                    html.P(
                                        "Number of Runs: {}".format(len(activities)),
                                        className="card-text",
                                    ),
                                    html.P(
                                        "Total Elevation Gain: {:.2f} m".format(
                                            activities["elevation_gain"].sum()
                                        ),
                                        className="card-text",
                                    ),
                                    html.P(
                                        "Average Pace: {:.2f} min/km".format(
                                            stats["average_pace"]
                                        ),
                                        className="card-text",
                                    ),
                                    html.P(
                                        "Month with Most Activities: {} ({} activities)".format(
                                            stats["month_with_most_activities"],
                                            stats["most_activities_count"],
                                        ),
                                        className="card-text",
                                    ),
                                ]
                        ...
                        dbc.Card(
                            dbc.CardBody(
                                [
                                    ...
                                    html.P(
                                        "5km PB: {}".format(stats["pb_5km"]),
                                        className="card-text",
                                    ),
                                    html.P(
                                        "10km PB: {}".format(stats["pb_10km"]),
                                        className="card-text",
                                    ),
                                    html.P(
                                        "Half Marathon PB: {}".format(
                                            stats["pb_half_marathon"]
                                        ),
                                        className="card-text",
                                    ),
                                    html.P(
                                        "Marathon PB: {}".format(stats["pb_marathon"]),
                                        className="card-text",
                                    ),
                                    html.P(
                                        "Longest Run: {:.2f} km".format(
                                            stats["longest_run"]
                                        ),
                                        className="card-text",
                                    ),
                                    html.P(
                                        "Highest Elevation Gain: {:.2f} m".format(
                                            stats["highest_elevation"]
                                        ),
                                        className="card-text",
                                    ),
                                ]
